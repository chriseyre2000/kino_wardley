...
export function extractNameLocationAndOffset(from) {
  return /\w+\s(?<name>[^\[]*)\[(?<first>[^\,]*),\s*(?<second>[^\]]*)]\s*[label\s\[]*(?<third>[^,]*)[,\s]*(?<fourth>[^\]]*)/.exec(from).groups
}
...
function drawAxis(svg, xaxis, yaxis, padding, labels) {
...
export function init(ctx, spec) {
  ...
  drawAxis(svg, xaxis, yaxis, padding, summary["__evolution"])
