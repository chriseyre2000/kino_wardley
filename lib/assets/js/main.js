...
export function parseSpec(specLines) {
  let summary = specLines.reduce((acc, i) => {
    ...
    if (item.startsWith("buy ")) {
      const name = item.replace("buy ", "");
      if (acc[name]) {
        acc[name].size = "buy";
      } 
    }

    if (item.startsWith("outsource ")) {
      const name = item.replace("outsource ", "");
      if (acc[name]) {
        acc[name].size = "outsource";
      } 
    }
    ...
    if (item.startsWith("component ")) {
      ...
      const dx = parseInt(details.third || "0");
      const dy = parseInt(details.fourth || "0");
      const inertia = item.includes(" inertia ") || item.endsWith(" inertia");
      const xor = item.includes(" xor ") || item.endsWith(" xor")

      acc[name] = {name: name, x: x, y: y, dx: dx, dy: dy, inertia: inertia, xor: xor, type: "component", size: "normal" }
    }
    ...
    if (item.startsWith("market ")) {
      ...
      acc[name] = {name: name, x: x, y: y, dx: dx, dy: dy, inertia: inertia, type: "market" }
    }
    ...
    if (item.startsWith("or ")) {
      const groups = /or*\s(?<name>[^\[]*)\[*(?<sy>[^,]*)(,\s)*(?<sx>[^,]*)(,\s)*(?<ey>[^,]*)(,\s)*(?<ex>[^\]]*)(\]\s)*(?<fill>[^\s]*)(\s*)(?<border>.*)/.exec(item).groups;
      const name = groups.name.trim();
      const sx = parseFloat(groups.sx || '0');
      const sy = parseFloat(groups.sy || '0');
      const ex = parseFloat(groups.ex || '0');
      const ey = parseFloat(groups.ey || '0');
      const fill = groups.fill;
      const border = groups.border || fill;

      acc[name] = {name: name, sx: sx, sy: sy, ex: ex, ey: ey, fill: fill, border: border, type: "or"}
    }
  ...
  specLines.forEach((item) => {
    ...
    if (item.includes("-v>")) {
      ...
      if(summary[parts[0].trim()] != undefined && summary[parts[1].trim()] != undefined) {
        ...
        .attr("x1", xaxis(end.x))
        .attr("y1", yaxis(end.y) )
        .attr("x2", xaxis(end.x + (0.1 * (start.y - end.y))))
        .attr("y2", yaxis(end.y + (0.1 * (end.x - start.x))))
        .attr("transform", (d) =>  `rotate(-45 ${xaxis(end.x)} ${yaxis(end.y)})`);
        ...
        .attr("x2", xaxis(end.x + (0.1 *(start.y - end.y))))
        .attr("y2", yaxis(end.y + (0.1 *(end.x - start.x))))
        .attr("transform", (d) =>  `rotate(-135 ${xaxis(end.x)} ${yaxis(end.y)})`)
      }
    }
...
function drawMarketCircle(svg, xaxis, yaxis, x, y) {
        //Outer
        svg.append("circle")
        .attr("cx", xaxis(x))
        .attr("cy", yaxis(y))
...
function fillColourBySize(size) {
  if (size === "buy") return "grey";
  if (size === "build") return "darkgrey";
  return "black";
}
...
function circlesAndText(svg, xaxis, yaxis, summary) {
  ...
  for (const [key, item] of Object.entries(summary)) {
    ...
    if (item.type === "component") {
      ...
      if (item.xor) {
        svg.append("rect")
        .attr('x', xaxis(item.x) - 3)
        .attr('y', yaxis(item.y) - 3)
        .attr('width', 6)
        .attr('height', 6)
        .attr('stroke', 'black')
        .attr('fill', 'white'); 
        ...
        svg.append("line")
        .style("stroke", "black")
        .attr("x1", xaxis(item.x) - 3)
        .attr("y1", yaxis(item.y) - 3)
        .attr("x2", xaxis(item.x) + 3)
        .attr("y2", yaxis(item.y) + 3);
        ...
        if (item.size !== "normal")
        {
          ...
          .attr("r", 15)
          .style("stroke", "black")
          .style("fill", fillColourBySize(item.size));
        }
    ...
    if (item.type === "market") {
      if(summary[item.name + "__evolve"] != undefined) {
        ...
        drawMarketCircle(svg, xaxis, yaxis, evolved.x, evolved.y)
      
        svg.append('text')
        .attr("x", xaxis(evolved.x) + 7 + evolved.dx)
        .attr("y", yaxis(evolved.y) + 5 + evolved.dy)
        .text(() => item.name)
      }

      drawMarketCircle(svg, xaxis, yaxis, item.x, item.y)
...
function drawGroups(svg, xaxis, yaxis, summary) {
  for (const [_key, item] of Object.entries(summary)) {
    ...
    if (item.type === "or") {
      if (item.border !== "") {
        svg.append('rect')
        .attr('x', xaxis(item.sx))
        .attr('y', yaxis(item.sy))
        .attr('width', xaxis(item.ex - item.sx ))
        .attr('height', xaxis(item.ey - item.sy ))
        .attr('stroke', item.border)
        .attr('fill', item.fill);      
      }
    }
