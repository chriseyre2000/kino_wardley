...
export function parseSpec(specLines) {
  let summary = specLines.reduce((acc, i) => {
    ...
    if (item.startsWith("annotation ")) {
      ...
      acc[label] = {label: label, description: description, points: buildPoints(locations) }
    }
...
export function buildPoints(input) {
  ...
  return parts.reduce(function(result, _, index, array) {
    if (index % 2 === 0)
      result.push( {x: parseFloat(array[index]), y: parseFloat(array[index + 1])}  );
    return result;
  }, []);

}

function drawAxis(svg, xaxis, yaxis, padding, labels, title) {
